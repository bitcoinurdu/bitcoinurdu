...
export default function ActiveAirdropsPage() {
  ...
  useEffect(() => {
    fetchCmsData().then((data) => {
      ...
      setAirdrops(all.filter((a) => a.status === 'active'));
      setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f]">
      <div className="max-w-6xl mx-auto px-4 py-12">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Active Airdrops</h1>
          <p className="text-gray-400">Currently active airdrops you can participate in right now.</p>
        </div>
        {loading ? (
          ...
          <Card className="border-[#1e1e2e] bg-[#12121a]"><CardContent className="p-8 text-center text-gray-400">No active airdrops found. Check back later!</CardContent></Card>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {airdrops.map((a) => (
              <Link key={String(a.id)} href={`/airdrops/${a.id}`}>
                <Card className="border-[#1e1e2e] bg-[#12121a] hover:border-green-500/30 transition-all cursor-pointer h-full">
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="font-bold text-white">{String(a.title)}</h3>
                      <Badge variant="outline" className="text-green-400 border-green-500/30 text-[10px]"><Clock className="h-2 w-2 mr-1" /> ACTIVE</Badge>
                    </div>
